...
string& Virkemanager::AllDataVirke()
{
	...
	if(!this->stringArrayGotInit)
	{
		cout <<"virkemanager: array not alloc before."<<endl;
...
string& Virkemanager::AllDataReglar()
{
	...
	if(!this->stringArrayGotInit)
	{
		cout <<"virkemanager: array not alloc."<<endl;
		string *tmpCache= new string[this->numberOfReglar()];
		this->stringArrayGotInit=true;
	}
	...
	for(int i; i < this->counterVirke; i++)
	{
		...
		if(typeid(*this->ptr2ArrayWithVirkePtr[i])== typeid(reglar))
		{
			...
			//cout for debug only remove or comment out
			std::cout << this->ptr2ArrayWithVirkePtr[i]->tostring() << " \n" << this->ptr2ArrayWithVirkePtr[i]->toStringSpec() << std::endl;
